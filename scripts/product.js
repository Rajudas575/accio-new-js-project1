...
const showProdcut = (product) => {
  ...
  const addToCartEl = $("#add_to_cart");

  const { title, id, price, description, rating, category, image } = product;
  ...
  addToCartEl.on("click", function (e) {
    const path = location.href;
    location.href = path.replace("product", "addcart") + `?id=${id}`;
  });
};
